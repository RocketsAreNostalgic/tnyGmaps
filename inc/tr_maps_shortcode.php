 ...
 * @var string $trmap_map_id [A 13 charecter id key so mutiple maps may be used on the same page.]
 * @var array $attr {
...
function trmap_mapme($attr)
{
    ...
    $trmap_map_id = uniqid('tr_map_'); // generate a unique 
    global $debug;
    // default atts
    ...
    $trmap_w       = ((substr($attr['w'], -2) != 'px') && (substr($attr['w'], -1) != '%')) ? $attr['w'] . 'px' : $attr['w'];
    $trmap_h       = ((substr($attr['h'], -2) != 'px') && (substr($attr['h'], -1) != '%')) ? $attr['h'] . 'px' : $attr['h'];
    $trmap_maptype = $attr['maptype'];
    
    $trmap_marker        = $attr['marker'];
    $trmap_icon          = $attr['icon'];
    $trmap_infowindow    = $attr['infowindow'];
    $trmap_infowindowb64 = $attr['infowindowb64'];
    
    $trmap_infowindowdefault = ($attr['infowindowdefault'] == 'true' || $attr['infowindowdefault'] == 'yes') ? 'yes' : 'no';
    $trmap_hidecontrols      = ($attr['hidecontrols'] == 'true') ? true : false;
    $trmap_scalecontrol      = ($attr['scale'] == 'true') ? true : false;
    $trmap_scrollwheel       = ($attr['scrollwheel'] == 'true') ? true : false;
    $trmap_refresh           = ($attr['refresh'] == 'true') ? true : false;
    $trmap_debug             = ($attr['debug'] == 'true') ? true : false;
    ...
    if ($attr['placeref'] != '' && ($attr['lat'] == '' || $attr['lng'] == '') && $attr['address'] == '') {
        ...
        $attr['address'] = null;
        (array) $attr = tr_map_get_place($api_key, $attr['placeref'], $attr['address'], $trmap_refresh, $trmap_debug);
        
    } elseif ($attr['placeref'] == '' && ($attr['lat'] == '' || $attr['lng'] == '') && $attr['address'] != '') {
        // here we have a address so get/set transient with fetched values		
        (array) $attr = tr_map_get_place($api_key, null, $attr['address'], $trmap_refresh, $trmap_debug);
        ...
        if ($attr['address'] == '' || $attr['address'] == ",") {
            ...
            if ($debug == true && current_user_can('edit_posts'))
                echo __('<b>MAP PLUGIN NOTICE:</b> Insufficiant location information.', 'trmaps');
            
        } else {
            (array) $hold = tr_map_get_place($api_key, null, $attr['address'], $trmap_refresh, $trmap_debug);
        ...
        if ($debug == true && current_user_can('edit_posts'))
            echo __("<b>MAP PLUGIN NOTICE:</b> Conflicting input values.<br> Include either a google <em>placeref</em> <b>OR</b> <em>address</em>, <b>OR</b> explicit <em>lat, lng</em> values <b>WITH</b> explicit location values: <em>name, street, city, state, postcode, country, phone, web.</em></br>", 'trmaps');
        return '';
    }
    
    // process the infowindow extras
    $trmap_infowindow_extras = ($trmap_infowindowb64 != '') ? base64_decode($trmap_infowindowb64) : '';
    // add any content from the basic to the end in its own div
    $trmap_infowindow_extras = ($trmap_infowindow != '') ? $trmap_infowindow_extras . '<div>' . $trmap_infowindow . '</div>' : $trmap_infowindow_extras;
    // convert the html special chars
    $trmap_infowindow        = htmlspecialchars_decode($trmap_infowindow_extras, ENT_QUOTES);
    // pass it through KSES to scrub it from unwanted markup
    $trmap_infowindow        = info_window_sanitize($trmap_infowindow);
    // Assemble the infowindow components
    $trmap_infowindow        = get_info_bubble($trmap_icon, $attr['name'], $attr['street'], $attr['city'], $attr['region'], $attr['postcode'], $attr['country'], $attr['phone'], $attr['web'], $trmap_infowindow);
    ...
    $returnme .= '<div class="tr_maps_wrap_' . $trmap_map_id . ' tr_maps_wrap">';
    $returnme .= '<div class="tr_map_canvas" id="' . $trmap_map_id . '" style="width:' . $trmap_w . '; height:' . $trmap_h . ';"></div>';
    $returnme .= '<div class="tr_maps_link_wrap"><a href="https://maps.google.com/?z=' . $trmap_z . '&q=' . $linkAddress . '&f=d&t=m"  target="_blank">open map in new window</a></div>';
    $returnme .= '</div>';
    $returnme .= '<script type="text/javascript">
		...
		var mapOptions = {
			zoom: ' . $trmap_z . ',
			center: latlng,
			scrollwheel: "' . $trmap_scrollwheel . '",
			scaleControl: "' . $trmap_scalecontrol . '",
			disableDefaultUI: "' . $trmap_hidecontrols . '",
			mapTypeId: google.maps.MapTypeId.' . $trmap_maptype . '
		};
		var ' . $trmap_map_id . ' = new google.maps.Map(document.getElementById("' . $trmap_map_id . '"), mapOptions);
		';
    //add custom image
    if ($trmap_marker != '') {
        $returnme .= 'var image = "' . $trmap_marker . '";';
    }
    $returnme .= '
    ...
    $returnme .= '
			position: ' . $trmap_map_id . '.getCenter()
		});
		';
    ...
    if ($trmap_infowindow) {
        $returnme .= '
			var contentString = \'' . $trmap_infowindow . '\'; // Here is my comment
			...
			google.maps.event.addListener(marker, \'click\', function() {
			  infowindow.open(' . $trmap_map_id . ',marker);
			});
			';
        //infowindow default
        if ('yes' == $trmap_infowindowdefault) {
            $returnme .= '
					infowindow.open(' . $trmap_map_id . ',marker);
				';
        }
...
function get_info_bubble($icon, $name, $street, $city, $state, $post, $country, $phone, $web, $info)
{
    ...
    $trmap_infowindowPlace = '<div class="marker inside"  >';
    $trmap_infowindowPlace .= '<b>' . $name . '</b>';
    $trmap_infowindowPlace .= '<table>';
    $trmap_infowindowPlace .= '<tbody>';
    $trmap_infowindowPlace .= '<tr>';
    $trmap_infowindowPlace .= '<td>';
    $trmap_infowindowPlace .= '<div class="" style="' . $iconStyle . '" >';
    $trmap_infowindowPlace .= ($street != null && $street != '' && $street != '') ? '<div>' . $street . '</div>' : '';
    $trmap_infowindowPlace .= ($city != null && $city != '' && $city !== '') ? '<div>' . $city . ', ' : '<div>';
    $trmap_infowindowPlace .= ($state != null && $state != '' && $state != '') ? $state . '</div>' : '</div>';
    $trmap_infowindowPlace .= ($post != null && $post != '' && $post != '') ? '<div>' . $post . '</div>' : '</div>';
    $trmap_infowindowPlace .= ($country != null && $country != '' && $country != '') ? '<div>' . $country . '</div>' : '</div>';
    $trmap_infowindowPlace .= ($phone != null && $phone != '' && $phone != '') ? '<div>' . $phone . '</div>' : '';
    $trmap_infowindowPlace .= ($web != null && $web != '' && $web != '') ? '<div style="max-width: 100%; white-space: nowrap; width: 100%; overflow: hidden;  -o-text-overflow: ellipsis;  text-overflow: ellipsis;"><a href="' . $web . '" class="gmap_link" target="_blank" style="">' . $web . '</a></div>' : '';
    $trmap_infowindowPlace .= '</div>';
    $trmap_infowindowPlace .= '</td>';
    $trmap_infowindowPlace .= '<td>';
    $trmap_infowindowPlace .= ($icon != null && $icon != '') ? '<img src="' . $icon . '" class="marker icon" style="margin: 0 5px 15px 5px; width: 60px; height: auto; " />' : '';
    $trmap_infowindowPlace .= '</td>';
    $trmap_infowindowPlace .= '</tr>';
    $trmap_infowindowPlace .= '</tbody>';
    $trmap_infowindowPlace .= '</table>';
    $trmap_infowindowPlace .= ($info != null && $info != '') ? '<div class="marker extras" style="border-top: 1px dotted #949494; margin-top: 5px; max-width: 265px; min-height: 40px; overflow: hidden; white-space: pre-wrap;" >' . $info . '</div>' : '';
    $trmap_infowindowPlace .= '</div>';
    return $trmap_infowindowPlace;
}
...
function tr_map_get_place($api_key, $placeRef, $address = '', $force_refresh, $plugin_debug)
{
    ...
    if ($force_refresh || false === $location) {
        if ($placeRef != '' && $placeRef != null) {
            ...
            if (!$api_key) {
                exit(__("<b>MAP PLUGIN NOTICE:</b> Google API Key has not been set and we cannot process a Places API refrence without it. See documentation on how to get and set your own key.", 'trmaps'));
            ...
            if ($debug == true && current_user_can('edit_posts')) {
                echo __("<b>MAP PLUGIN NOTICE:</b> There doesn't seem to be a location or address, check that the shortcode is formed propperly.", 'trmaps');
            }
            return '';
        }
        ...
        if (is_wp_error($response)) {
            if ($debug == true && current_user_can('edit_posts')) {
                echo __("<b>MAP PLUGIN NOTICE:</b> We recieved an error in the responce: </br>", 'trmaps');
                echo "<pre>";
                print_r($response);
                echo "</pre>";
            }
        ...
        if (is_wp_error($data)) {
            if ($debug == true && current_user_can('edit_posts')) {
                echo __("<b>MAP PLUGIN NOTICE:</b> There were problems retrieving the body of the responce. ", 'trmaps');
            }
            return '';
        }
        if ($response['response']['code'] == 200) {
            ...
            if ($data->status === 'OK') {
                if ($debug == true && current_user_can('edit_posts')) {
                    echo __("<b>MAP PLUGIN NOTICE:</b> Status OK", 'trmaps');
                    echo "<pre>";
                    print_r($data);
                    echo "</pre>";
                }
                if ($plugin_debug == true && current_user_can('edit_posts')) {
                    echo __("<b>Google Responce:</b>", 'trmaps');
                    echo "<pre>";
                    echo $debug;
                    print_r($data);
                    echo "</pre>";
                ...
                if ($debug == true && current_user_can('edit_posts'))
                    echo __('<b>MAP PLUGIN NOTICE:</b> Server-side error, please try again in a short while.', 'trmaps');
                return '';
            } elseif ($data->status === 'ZERO_RESULTS') {
                if ($debug == true && current_user_can('edit_posts'))
                    echo __('<b>MAP PLUGIN NOTICE:</b> No location found for the entered place reference. This may indicate the location may have changed names, or is no longer opperating.', 'trmaps');
                return '';
            } elseif ($data->status === 'OVER_QUERY_LIMIT') {
                if ($debug == true && current_user_can('edit_posts'))
                    echo __('<b>MAP PLUGIN NOTICE:</b> This API Key is over its quota.', 'trmaps');
                return '';
            } elseif ($data->status === 'REQUEST_DENIED') {
                if ($debug == true && current_user_can('edit_posts'))
                    echo __('<b>MAP PLUGIN NOTICE:</b> Request Denied. Usually this is due to the sensor parameter being missing in the search string..', 'trmaps');
                return '';
            } elseif ($data->status === 'INVALID_REQUEST') {
                if ($debug == true && current_user_can('edit_posts'))
                    echo __('<b>MAP PLUGIN NOTICE:</b> Invalid request. Did you enter a location reference?', 'trmaps');
                return '';
            } elseif ($data->status === 'NOT_FOUND') {
                if ($debug == true && current_user_can('edit_posts'))
                    echo __('<b>MAP PLUGIN NOTICE:</b> Place not found. Usually this is due to an incomplete or corrupted reference string.', 'trmaps');
                return '';
            } else {
                if ($debug == true && current_user_can('edit_posts'))
                    echo __('<b>MAP PLUGIN NOTICE:</b> Something went wrong while retrieving your map, please ensure you have entered the short code correctly.', 'trmaps');
                return '';
            }
        } else {
            $responseCode = $response['response']['code'];
            if ($debug == true && current_user_can('edit_posts'))
                echo __('Unable to contact Google API, service response: ' . $responseCode, 'trmaps');
            return '';
        }
    } else {

...
$pluginfolder = plugins_url('', dirname(__FILE__));
$pluginfolder = preg_replace('!http(s)?://' . $_SERVER['SERVER_NAME'] . '/!', '/', $pluginfolder);
...
/*
 * set up and populate variables
 */

var apiTest =  '<?php echo(defined('GOOGLEMAPS_API_KEY') && (constant('GOOGLEMAPS_API_KEY') != null || constant('GOOGLEMAPS_API_KEY') != '')) ?  true :  false; ?>';
var lat = null;
var lng = null;
...
var markerCustom = '<?php echo $pluginfolder; ?>/icons/music_folk_map.png';
...
function initialize(infowindow) {
	...
	jQuery(document).ready(function($) {
		...
		jQuery('.heading').click(function () {
			...
			if (jQuery(this).next().find('.accordian').hasClass('open')){
			...
			// close all others
		    jQuery('.heading').not(this).next('.group').find('.accordian').slideUp(function(){
		    	jQuery(this).removeClass('open active');
		    	setFocus(jQuery(this));
		    });
		    // open the cliked one if its closed
		    jQuery(this).next().find('.accordian').slideDown(400, function(){
		    	setFocus(jQuery(this));
	    ...
	    jQuery('input[name="custom_use_address_check"]').click(function() {
		    if( jQuery(this).is(':checked')) {
	        	jQuery('#mapAddress-group').slideUp(400, function(){
		        	jQuery('#address_extras').find('input:text, button').addClass('active hilight').prop('disabled', false).delay(700).show( 1 , function(){
	    ...
	    jQuery('.map-icon').click(function(e){
	    	...
	    	jQuery('input#mapMarkerImage').val('<?php echo $pluginfolder; ?>/icons/' + icon );
	    	update_marker();
		});
	    // Notice
	...
	function updateMapInfoWindow () {
        ...
        infowindowPlace = get_info_bubble(locIcon, locName, locStreet, locCity, locRegion, locPostcode, locCountry, locPhone, locWeb, mapInfoWindowReturn);
		infowindow.setContent(infowindowPlace);
		jQuery('textarea#mapInfoWindow').prop('disabled', true).addClass('hilight alert').delay(700).show( 1 , function(){
			  jQuery(this).val(mapInfoWindowReturn).removeClass('hilight alert').prop('disabled', false);
...
tinyMCEPopup.onInit.add(trGmaps.init, trGmaps);
/*
 ...
 Use and distribution http://www.opensource.org/licenses/bsd-license.php
...
(
	function ( $ ) {
				...
				if ( tag.isClosing ) {
					...
					for ( var repIndex = 0; repIndex < options.replace.length; repIndex ++ ) {
						for ( var tagIndex = 0; tagIndex < options.replace[repIndex][0].length; tagIndex ++ ) {
							var byName = typeof (
									options.replace[repIndex][0][tagIndex]
								) == "string";
							if ( (
							...
							value =
								$.grep( value.split( " " ), function ( c ) {
									 return $.grep( options.allowedClasses, function ( a ) {
											 return a[0] == c && (
												 a.length == 1 || $.inArray( element.tag.name, a[1] ) > - 1
												 );
										 } ).length > 0;
								 } )
								 .join( " " );
		...
		function Tag( name, close, rawAttributes, options ) {
			...
			this.isClosing = (
			close != undefined && close.length > 0
			);

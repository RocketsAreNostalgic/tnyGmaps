	...
	<body>
		...
		<div id="button-dialog" class="wrap">
			...
			<? } else { ?>
				...
				<div class="group">
						<div id="address_extras" class="accordion" style="display:none;">
							...
							<div class="input ">
								...
								<input type="text" placeholder="<?php _e('Joes Music Emporium', 'tiny_gmap'); ?>" name="locName"  id="locName" class="noEnterSubmit" disabled/>
							</div>
							<div class="input float large">
								<label for="locStAdr"><?php _e('Street', 'tiny_gmap'); ?>:</label>
								<input type="text" name="locStAdr"  placeholder="<?php _e('32 Somewhere Drive', 'tiny_gmap'); ?>" id="locStAdr"  class="noEnterSubmit" disabled/>
							</div>
							<div class="input float med">
								<label for="locCity"><?php _e('City', 'tiny_gmap'); ?>:</label><br/>
								<input type="text" placeholder="<?php _e('Greenville', 'tiny_gmap'); ?>" name="locCity"  id="locCity" class="noEnterSubmit" disabled />
							</div>
							<div class="input float thirds">
								<label for="locRegion"><?php _e('Region/State', 'tiny_gmap'); ?>:</label>
								<input type="text" placeholder="<?php _e('Our State', 'tiny_gmap'); ?>"  name="locRegion"  id="locRegion" class="noEnterSubmit" disabled/>
							</div>
							<div class="input float thirds">
								<label for="locPostcode"><?php _e('Postcode', 'tiny_gmap'); ?>:</label>
								<input type="text" placeholder="<?php _e('POSTCODE', 'tiny_gmap'); ?>"  name="locPostcode"  id="locPostcode" class="noEnterSubmit" disabled/>
							</div>
							<div class="input float thirds">
								<label for="locCountry"><?php _e('Country', 'tiny_gmap'); ?>:</label>
								<input type="text" placeholder="<?php _e('Proud Country', 'tiny_gmap'); ?>"  name="locCountry"  id="locCountry" class="noEnterSubmit" disabled/>
							</div>
								<div class="input float large">
							...
							<div class="input float thirds">
								<button name="lookup" type="submit" class="buttn bttn-submit" id="lookup-detials" onclick="return false;" ><?php _e('Lookup address', 'tiny_gmap'); ?></button>
							</div>
							<div class="input float thirds">
								<button name="update" type="submit" class="buttn" id="map-update" onclick="return false;" ><?php _e('Update info widow', 'tiny_gmap'); ?></button>
							</div>
							<div class="input float thirds">
								<button name="clear-fields" type="submit" class="buttn bttn-warning" id="clear-fields" onclick="return false;" ><?php _e('Clear feilds', 'tiny_gmap'); ?></button>
							</div>
							<div class="input">
				...
				<label class="heading"><?php _e('Map attributes', 'tiny_gmap'); ?>:<span data-tooltip="<?php _e('Use these settings to adjust the appearence of your map.', 'tiny_gmap'); ?>" >?</span></label>
				<div class="group" >
						...
						<div id="map_extras" class="accordion" style="display:none;">
							...
							<div class="detial-notice">
								<small><em><b><?php _e('Specify size in &#039;px&#039; or &#039;%&#039;', 'tiny_gmap'); ?></b></em></small>
							</div>
							...
							<div class="mapMarkerImage_wrap">
								<div class="input">
									<label class="heading" for="mapMarkerImage"><?php _e('Custom icon URL', 'tiny_gmap'); ?>: <span data-tooltip="<?php _e('List the full (or root realitive) path to your custom icon, or select from the list below. If your custom icon won&#039;t load, then there is probably a typo in your path.', 'tiny_gmap'); ?>">?</span></label>
									<input type="text" name="mapMarkerImage" placeholder="<?php echo $local_uri; ?>/icons/" value="<?php echo $local_uri; ?>/icons/<?php echo($loaded_icons[0]); ?>" id="mapMarkerImage" />
									<span class="icon-list alert" ><em>Loaded icons:</em> <br /><?php print($icons_list) ?>
									<p><em><?php _e('Icons courtesy of the', 'tiny_gmap'); ?> <a href="http://mapicons.nicolasmollet.com/" target="_blank"><?php _e('Map Icons Collection', 'tiny_gmap'); ?></a></em></p>
									</span> 
								</div>
							</div>
							<div class="detial-notice">
								<small><em><br><?php _e('Hight &amp; width settings are not reflected in map preview.', 'tiny_gmap'); ?></em></small>
							</div>
						</div>	
				</div>
			</form>

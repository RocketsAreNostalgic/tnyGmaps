...
function processAddressObject( needle, haystack ) {
	...
	for ( var i = 0; i < haystack.length; i ++ ) {
		var addr = haystack[i];
		rtrn += (
			addr.types[0] === needle ? addr.long_name : ''
		);
	}
	return rtrn;
}
 ...
 *
 */
function empty_location_fields() {
...
function initialize( infowindow ) {
	...
	function generateMap() {
		...
		seed_vars();
		initCenter = (
			lat !== "" && lng !== "" ? new google.maps.LatLng( lat, lng ) : new google.maps.LatLng( 43.703793, - 72.326187 )
		);
		mapOptions = {
			center: initCenter,
			zoom: mapZoomReturn,
	...
	function addListeners() {
		...
		google.maps.event.addDomListener( document.getElementById( "locName" ), "change", empty_location_fields );
		google.maps.event.addDomListener( document.getElementById( "locPhone" ), "change", empty_location_fields );
		google.maps.event.addDomListener( document.getElementById( "locStAdr" ), "change", empty_location_fields );
		google.maps.event.addDomListener( document.getElementById( "locCity" ), "change", empty_location_fields );
		google.maps.event.addDomListener( document.getElementById( "locRegion" ), "change", empty_location_fields );
		google.maps.event.addDomListener( document.getElementById( "locPostcode" ), "change", empty_location_fields );
		google.maps.event.addDomListener( document.getElementById( "locCountry" ), "change", empty_location_fields );
		google.maps.event.addDomListener( document.getElementById( "locWebsite" ), "change", empty_location_fields );
	...
	function updateMapAutocomplete() {
		...
		if ( locPlace.address_components ) {
			...
			locGooglePlaceID = (
				locPlace.place_id ? locPlace.place_id : ""
			);
			...
			// Do the same for nested address_components -- iterate through the array, than set the field value
	...
	function updateCustomlocation() {
		if ( jQuery( "#search-report" ).is( ":animated" ) ) {
			...
			empty_location_fields(); // clear the custom bits
...
tnyGmapsAssembleShortcode = {
	...
	insert: function insertButton( ed ) {
		...
		markerOutput += (
			mapControlsReturn === true ? 'hidecontrols="true" ' : ""
		);
		...
		if ( locGooglePlaceID === null || locGooglePlaceID === "" ) {
			...
			markerOutput += (
				locName !== "" ? 'name="' + htmlEntities( locName ) + '" ' : ""
			);
			markerOutput += (
				locStreet !== "" ? 'street="' + htmlEntities( locStreet ) + '" ' : ""
			);
			markerOutput += (
				locCity !== "" ? 'city="' + htmlEntities( locCity ) + '" ' : ""
			);
			markerOutput += (
				locRegion !== "" ? 'region="' + htmlEntities( locRegion ) + '" ' : ""
			);
			markerOutput += (
				locPostcode !== "" ? 'postcode="' + htmlEntities( locPostcode ) + '" ' : ""
			);
			markerOutput += (
				locCountry !== "" ? 'country="' + htmlEntities( locCountry ) + '" ' : ""
			);
			markerOutput += (
				locWeb !== "" ? 'website="' + locWeb + '" ' : ""
			);
			markerOutput += (
				locPhone !== "" ? 'phone="' + htmlEntities( locPhone ) + '" ' : ""
			);
			markerOutput += (
				locIcon !== "" ? 'icon="' + htmlEntities( locIcon ) + '" ' : ""
			);
		} else {
		...
		if ( parent.tnygmaps.custom_icon === mapMarkerImageReturn ) {
			...
			markerOutput += (
				mapMarkerReturn !== "" && mapMarkerImageReturn !== "" ? 'marker="' + mapMarkerImageReturn + '" ' : ""
			);
		}
		markerOutput += (
			mapMarkerReturn !== "" && mapInfoWindowReturn !== "" ? 'infowindowb64="' + mapInfoWindowReturn + '" ' : ""
		);
